...
int16_t OPLL_calc(OPLL *opll) {
  ...
  // Unfortunately I have to comment this out due to the fact that the 
  // RP2040 could not handle all these resampling (requires hardware FPU):
  ...
  // Without these resampling steps - the output frequency may be very off!
  // Adding another pair of update_output and mix_output results in a higher output frequency! 
  update_output(opll);
  mix_output(opll);
